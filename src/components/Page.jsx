...
const easingFactor = 0.5; // controls the speed of the interpolation
...
pages.forEach((page) => {
  ...
  useTexture.preload(`/textures/book-back-roughness.webp`);
});
...
const Page = ({ number, front, back, page, opened, bookClosed, ...props }) => {
  ...
  const [picture, picture2, pictureRoughness] = useTexture([
    ...
    ...(number === 0 || number === pages.length - 1
      ? [`/textures/book-back-roughness.webp`]
      : []),
  ]);
  ...
  const manualSkinnedMesh = useMemo(() => {
    ...
    for (let i = 0; i <= PAGE_SEGMENTS; i++) {
      ...
      if (i > 0) {
        bones[i - 1].add(bone); // attach new bone to the previous
      }
    }
  ...
  return (
    <group
      ...
      onClick={(e) => {
        ...
        if (materialIndex === 4) {
          // Click en la cara frontal → avanzar
          setPage((prev) => Math.min(prev + 1, pages.length));
        } else if (materialIndex === 5) {
          // Click en la cara trasera → retroceder
